...
################################# starting -all datas in account ######################################
@accounts.route("/all/accounts")
def get_allaccounts():
    account= Account.query.all()
    if not account:
            return jsonify(f"There is no datas or please wait until datas gets uploaded")

    return accounts_schema.dump(account)


@accounts.route('/all/address')
def get_alladdress():
    address= Address.query.all()
    if not address:
        return jsonify(f"There is no datas or please wait until datas gets uploaded")
...
@accounts.route('/all/rwood')
def get_allrwood():
    rwood= Rwood.query.all()
    if not rwood:
        return jsonify(f"There is no datas or please wait until datas gets uploaded")

    return rwoods_schema.dump(rwood)


@accounts.route('/all/industryinfo')
def get_allindustryinfo():
    industryinfo= IndustryInfo.query.all()
    if not industryinfo:
        return jsonify(f"There is no datas or please wait until datas gets uploaded")
 
    return industrys_schema.dump(industryinfo)


################################# Ending -all datas in account ######################################



################################# starting -adding datas in account ######################################
...
################################# Ending  adding datas in account ######################################


################################# starting -deleting  datas in account ######################################
...
################################# ENDING -deleting datas in account ######################################

################################# starting -updating datas in account ######################################
...
################################# Ending -updating datas in account ######################################
